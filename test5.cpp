...
void printPathesLen(acc_vv_t &vv) {
  ...
  for (acc_vv_t::size_type i = 0, len = vv.size(); i < len; ++i) {
    cout << "Path " << i << ", length: " << vv[i].size() << " nodes" << endl;
  }
}
...
void dropLessPathes(acc_vv_t &vv) {
  // Get max len
  acc_vv_t::size_type max = 0;
  for (auto v : vv) {
    max = v.size() > max ? v.size() : max;
  }

  acc_vv_t::iterator it = remove_if(vv.begin(), vv.end(),
                                    [&max](const acc_v_t &elem) {
                                      return (elem.size() < max) ? true : false;
                                    });
  vv.erase(it, vv.end());
}

TEST(allPathesFinderTest, onlyRoot) {
  TreeNode n0 = { 0 };
  acc_vv_t vv;
